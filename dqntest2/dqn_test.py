...
testgrid = [[2,2,4,4],[0,0,0,8],[0,0,0,0],[0,0,0,0]]

testonehot = processor.process_observation(np.array(testgrid))

teststate = dqn.memory.get_recent_state(testonehot)
testQ = dqn.compute_q_values(teststate)
testaction = dqn.test_policy.select_action(testQ)
# print(dqn.model.predict(testonehot, 1))
print(testaction)