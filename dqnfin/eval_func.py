...
def eval_func(state):
    ...
    # score = env.score
    tonic = tonicity(state)
    smooth = smoothness(state)
    # tiles = len(state[state != 0])
    empty_tiles = len(state[state == 0])
    # adis = avg_dis(state)
    # stat, c = get_count(state)
    # return max_tile + smooth + tonic + 2.7 * np.average(state)
    s = np.abs(max_tile) + np.abs(tonic) + np.abs(smooth) + np.abs(empty_tiles)
    # with open('output.csv', 'a') as f:
    #     f.write('{}, {}, {}, {}\n'.format(
    #         round(np.abs(max_tile/s), 2),
    #         round(np.abs(tonic/s), 2),
    #         round(np.abs(smooth/s), 2),
    #         round(np.abs(empty_tiles/s), 2)
    #     ))
    return max_tile + 0.6 * smooth + 8.0 * tonic + empty_tiles * np.average(state)